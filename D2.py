...
k = {
    ...
    'Большой': {'k': 3.07e-07, 'ksigma_k_S': 6.93e-09},
    'Холодный': {'k': 3.56e-07, 'k_sigma_kS': 1.15e-08}
}
S = {
...
shell_masses = {
    'Малый': 4.3, 'Средний': 4.38, 'Больший': 4.48, 'Холодный': 4.05
}
...
delta = {
    'Малый': (
        np.array([0.0, 179.0, 1472.0, 3290.0]),
        np.array([0.002502, 0.002613, 0.002866, 0.003154])
    ),
    'Средний': (
        np.array([0.0, 192.0, 1473.0, 3290.0]),
        np.array([0.001816, 0.001853, 0.001957, 0.002230])
    ),
    'Больший': (
        np.array([0.0, 185.0, 1476.0, 3198.0]),
        np.array([0.001729, 0.001794, 0.001940, 0.002215])
    ),
    'Холодный': (
        np.array([0.0, 190.0, 1480.0, 2614.0]),
        np.array([0.002575, 0.002652, 0.002967, 0.003272])
    )
}
...
balls = {
    'Малый': (time_maly_ch, diff_maly),
    'Средний': (time_sred_ch, diff_sred),
    'Большой': (time_bolsh_ch, diff_bolsh),
    'Холодный': (time_holod_ch, diff_holod)
}
...
# Словарь для сопоставления несовпадающих ключей ('Большой' / 'Больший')
key_map = {
    'Малый': 'Малый',
    'Средний': 'Средний',
    'Большой': 'Больший',
...
for name in ['Малый', 'Средний', 'Большой', 'Холодный']:
    geom_key = key_map[name]

    # Извлекаем геометрические параметры для t=0
    S_0 = S[geom_key][1][0]  # Стартовая площадь (см²)
    delta_0 = delta[geom_key][1][0]  # Стартовая толщина оболочки (см)
    k_val = k[name]['k']  # Константа скорости k (с⁻¹)

    # Данные кинетики взвешивания
    time_ch, diff_m = balls[name]  # Время в часах, разность масс в граммах

    # Расчёт D по формуле из картинки (все величины в СГС, D получается в см²/с)
    D_array = (k_val * delta_0 * diff_m) / (S_0 * rho_diff)
